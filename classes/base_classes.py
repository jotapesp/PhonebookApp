...
class UniqueList:
    ...
    def validIndex(self, index):
        return index >= 0 and index <= len(self.lista)
...
class TelephoneList(UniqueList):
    def __init__(self):
...
class Contact:
    ...
    def searchTelephone(self, telephone):
        pos = self.telephones.searchItem(Telephone(telephone))
        if pos == -1:
            return  None
        else:
            return self.telephones[pos]
